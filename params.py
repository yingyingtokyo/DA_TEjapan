...
########################
# *************************************************************
# define which version params.py
def def_params():
    pm_line = 'import params_ils01 as pm\n' 
    ...
    replace_as_pm_line('calc_end_hour.py')
...
def mode():
    ...
    # 9: off  -- turn on the offline DA mode
...
def starttime(DAhour=DA_leading()):
    ...
    return (2024,1,21,0) # start date: [year,month,date]
    #else:
    #    return (2022,1,1)

def endtime(DAhour=DA_leading()):
    #if DAhour == 'on':
    return (2024,12,30,0) # end date: [year,month,date]
...
def val_mode():
    ...
    # return 1  # turn on validation mode but no need to reselect stations and no need to download discharge for validation
...
def outdir():
    if (val_mode()==0):
        return '/data50/yingying/HydroDA/'+str(starttime(DAhour=DA_leading())[0])+'/out/'
    else:
        return '/data50/yingying/HydroDA/'+str(starttime(DAhour=DA_leading())[0])+'/out_valid/'

def expout():
    if (val_mode()==0):
        return '../../'+str(start_year)+'/out'
    else:
        return '../../'+str(start_year)+'/out_valid'
