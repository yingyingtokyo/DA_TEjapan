...
typeC = "C" # simulation
typeF = "F" # offline DA
output_path  = '/work/a06/yingying/Code/plot/'+exp+'/'
...
if var_name == 'outflw':
    ...
    lat,lon,disF_mean = read_ens_nc(output_path+var_off+'/data'+typeF+'_mean01_20.nc')
...
basin=read_bin('basin.bin')
up_area = read_bin('uparea.bin')
elvmean = read_bin('elv_mean.bin')
...
lat,lon,var = read_nc("/work/a06/yingying/CaMa_v411/cmf_v411_pkg/out/test-exp100/o_outflw2019.nc",3)
del var
...
obs_path = '/work/a06/yingying/obs/2019h/'+var_dir+'/'  
def read_obs(day,hr):
...
for day in range(day1,day2+1):
    for hr in range(1,25):
        obs[ind,:,:]  = read_obs(day,hr)
        ind = ind+1

#%%  draw the da output
os.makedirs('/work/a06/yingying/plot/'+exp+'/',exist_ok=True)
def draw_lines(loc_lat,loc_lon,ylabel,loc_sim_mean,loc_sim_max,loc_sim_min,loc_da,loc_obs,rain):
    ...
    dates = pd.date_range(start='2019-10-01 00:00',end='2019-10-18 23:00',freq='H')
    ax1.set_xticks(np.arange(0,time_len,24*3))
    ax1.set_xticklabels(dates[::24*3],rotation=15)
    ...
    # plt.show()
    os.makedirs('/work/a06/yingying/plot/'+exp+'/offline/',exist_ok=True)
    plt.savefig('/work/a06/yingying/plot/'+exp+'/offline/'+str(loc_lon)+str(loc_lat)+'.jpg', format='jpg',dpi=300)
    plt.close()

def draw_obs(obs_lat,obs_lon):
    simmean = np.nanmean(disC_mean,axis=0)
    damean  = np.nanmean(disF_mean,axis=0)
    for loc in range(0,np.shape(obs_lat)[0]):
        ...
        if var_name == 'outflw':
            ...
            dis_obs = obs[0:time_len,loc_lat,loc_lon]
        else:
            ...
            disA_da_mean = disF_mean[0:time_len,loc_lat,loc_lon]-damean[loc_lat,loc_lon]
            dis_obs = obs[0:time_len,loc_lat,loc_lon]-elvmean[loc_lat,loc_lon]
        #offline DA
        # disF_loc  = data[0:time_len,loc_lat,loc_lon]
        rain    = rainf[0:time_len,loc_lat,loc_lon]
        draw_lines(loc_lat,loc_lon,var_ylabel,disC_sim_mean,disC_sim_max,disC_sim_min,disA_da_mean,dis_obs,rain)

draw_obs([554-1],[959-1])
draw_obs([563-1],[963-1])
draw_obs([611-1],[1063-1])
draw_obs(ilat,ilon)
...
def draw_each_figure(obs_lat,obs_lon):
    ...
    for loc in range(0,np.shape(obs_lat)[0]):
        ...
        loc_da  = disA_mean[0:time_len,loc_lat,loc_lon]-damean[loc_lat,loc_lon]
        ...
        if len(where_nan>0):
            ...
            loc_sim   = np.delete(loc_sim,where_nan)
            loc_da    = np.delete(loc_da,where_nan)
        if len(loc_obs)==0 | len(loc_sim)==0 | len(loc_da)==0:
            continue
        print(loc_da)
...
def draw_rmse(title,situ_nrmse,obs_lon,obs_lat,vmin,vmax):
    ...
    con_plot2 = ax1.scatter(lon[obs_lon],lat[obs_lat],c=situ_nrmse,s=1.5,zorder=2,cmap='coolwarm',transform=ccrs.PlateCarree(),vmin=vmin,vmax=vmax)
    ...
    # plt.show()
    plt.savefig('/work/a06/yingying/plot/'+exp+'/off_'+title+'.jpg', format='jpg',dpi=600)
    # plt.close()
draw_rmse('RMSE',situ_rmse,lon_plot,lat_plot,-max(situ_rmse),max(situ_rmse))   
draw_rmse('nRMSE',situ_nrmse,lon_plot,lat_plot,-0.4,0.4)   
draw_rmse('Improvement',situ_imp,lon_plot,lat_plot,0,1)   
