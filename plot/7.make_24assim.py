...
#end_year,end_month,end_date,end_hour=(2019,10,14,0) # end year month date
...
if 'case' in pm_name:
    lon_point_ind = np.load(pm.plot_dir()+'/exp_'+expname+dahour_str+'/lon_case_assim'+pm_name[-4]+'.npy')
    lat_point_ind = np.load(pm.plot_dir()+'/exp_'+expname+dahour_str+'/lat_case_assim'+pm_name[-4]+'.npy')
else:
    lon_point_ind = np.load(pm.plot_dir()+'/exp_'+expname+dahour_str+'/lon_assim_ind.npy')
    lat_point_ind = np.load(pm.plot_dir()+'/exp_'+expname+dahour_str+'/lat_assim_ind.npy')
...
def read_24h_nc(enum,varname):
    ind = 0
    current_time = stime
    filepath = pm.DA_dir()+'/'+syyyy+'/out_'+exppath[7:10]+'_round0'+'/CaMa_out_ils'+dahour_str+'/'
    end_day = etime-timedelta(days=1)
...
def parallel(varname):
    ...
    for i,(da,sim) in enumerate(results):
        ...
        sim_new=sim
        if i>0:
            wlv_da  = np.nanmean(np.concatenate((da_old[np.newaxis,:],da_new[np.newaxis,:]),axis=0),axis=0)
            wlv_sim = np.nanmean(np.concatenate((sim_old[np.newaxis,:],sim_new[np.newaxis,:]),axis=0),axis=0)
            da_old  = wlv_da
            sim_old = wlv_sim
        else:
            da_old = da
            sim_old=sim
    if 'case' in pm_name and 'all' in pm_name:
        savepath = pm.plot_dir()+'/exp_'+expname+dahour_str+'/case_all'+pm_name[-4]+'/'+varname+'/'
    elif 'case' in pm_name and 'ils' in pm_name:
        savepath = pm.plot_dir()+'/exp_'+expname+dahour_str+'/case_ils'+pm_name[-5]+'/'+varname+'/'
    else:
        savepath = pm.plot_dir()+'/exp_'+expname+dahour_str+'/'+exppath[7:10]+'/'+varname+'/'
    os.makedirs(savepath, exist_ok=True)
    np.save(savepath+'da_24assim.npy',wlv_da)
    np.save(savepath+'sim_24assim.npy',wlv_sim)
parallel('rivdph')
parallel('outflw')
