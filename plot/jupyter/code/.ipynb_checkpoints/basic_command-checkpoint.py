...
import params_case11 as pm    # out_msm_round*  # params_all_case3
pm_path = pm.__file__
pm_name = pm.__name__
...
def evaluate_hour(obs_station,sim_station,wlv_da,hour_str,time_leng,lat_all_flood,lon_all_flood,obs_station_all,da_station_all,sim_station_all,tstart_fore,simmean,time_len,wlv_sim):
    ...
    matrix_da  = np.full(6,np.nan)
    matrix_sim = np.full(6,np.nan)
    # plot stations
    plot_idx = 0
    for ind, i in enumerate(valid_indices):
        ...
        if (np.shape(obs_cal)[0]!=np.shape(sim_cal)[0]) | (np.shape(da_cal)[0]!=np.shape(obs_cal)[0]) | (np.shape(sim_cal)[0]!=np.shape(da_cal)[0]):
            continue
        matrix_da[0],matrix_da[1],matrix_da[2],matrix_da[3],matrix_da[4] = cal_peak_matric(obs_cal,da_cal)
        matrix_sim[0],matrix_sim[1],matrix_sim[2],matrix_sim[3],matrix_sim[4] = cal_peak_matric(obs_cal,sim_cal)
        # calculate STD
        matrix_da[5]   = cal_STD(da_sta[:,time_sind-int(hour_str):time_eind-int(hour_str)])
        matrix_sim[5]  = cal_STD(sim_sta[:,time_sind-int(hour_str):time_eind-int(hour_str)])
        plot_idx = plot_idx + 1
    return matrix_da, matrix_sim
def evaluate_tot_hour(obs_station,sim_station,wlv_da,lat_all_flood,lon_all_flood,obs_station_all,da_station_all,sim_station_all,tstart_fore,simmean,time_len,wlv_sim):
    ...
    da_matrix_wlv  = np.full((24,6),np.nan)
    sim_matrix_wlv = np.full((24,6),np.nan)
    time_leng = 1
