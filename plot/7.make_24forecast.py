...
print(start_year,start_month,start_date,start_hour)
exit()
...
if 'case' in pmname and 'all' not in pmname:
    lat_point_ind= np.load(pm.plot_dir()+'/exp_'+expname+dahour_str+'/'+pmname[-6:]+'lat.npy')
    lon_point_ind= np.load(pm.plot_dir()+'/exp_'+expname+dahour_str+'/'+pmname[-6:]+'lon.npy')
else:
...
def read_24h_nc(enum,varname):
    ...
    if syyyy == '2024':
        ...
        filepath = pm.DA_dir()+'/'+syyyy+'/out_case1_round'+str(int(pmname[-1])+10)+'/CaMa_out_ils'+dahour_str+'/'
    else:
        filepath = pm.DA_dir()+'/out_'+exppath[7:10]+'_round0'+'/CaMa_out_ils'+dahour_str+'/'
