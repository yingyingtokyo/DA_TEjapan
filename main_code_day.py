...
def main_act():
    ...
    if pm.output_er()==1:
        os.system("rm -Rf ./"+pm.CaMa_out()+"/"+yyyy+"*")
        os.system("rm -Rf ./"+pm.CaMa_in()+"/"+pm.input()+"/Roff_CORR/Roff__"+yyyy+"*")
...
def one_day_loop(yyyy,mm,dd,day):
    ...
    if pm.output_er()==1:
        ...
        if bef_dt != datetime.date(bef_dt.year,12,31): # to keep restart file in last day of the year
            os.system("rm -Rf ./"+pm.CaMa_out()+"/"+bef_yyyy+bef_mm+bef_dd+"*")
...
def spinup_loop(inputlist):
    ...
    os.system("source "+str(pm.DA_dir())+"/src/spin_up.sh "+str(yyyy)+" "+str(loop)+" "+ens_num+" "+dir2+" "+str(cpunums)+" "+str(run_name)+" "+str(exp_dir)+" "+str(mapname)+" "+str(cal)+" "+str(pm.CaMa_out())+" "+str(pm.CaMa_in()))
    return 0
###########################
def one_day_sim(inputlist):
    ...
    +" "+DA_dir+" "+(pm.CaMa_out())+" "+(pm.CaMa_in())) #+str(corrupt)+" "
...
def copy_corrupted_sfcelv(inputlist):
    ...
    num  = inputlist[3]
    numch='%03d'%num
    fname="./"+pm.CaMa_out()+"/"+yyyy+mm+dd+"C"+numch+"/sfcelv"+yyyy+".bin"
    os.system("cp "+fname+" ./assim_"+(pm.runname(pm.mode()))+"/ens_xa/open/"+yyyy+mm+dd+"_"+numch+"_xa.bin")
    return 0
...
def copy_corrupted_restart(inputlist):
    ...
    numch='%03d'%num
    fname="./"+pm.CaMa_out()+"/"+yyyy+mm+dd+"C"+numch+"/restart"+n_yyyy+n_mm+n_dd+".bin"
    os.system("cp "+fname+" ./"+pm.CaMa_in()+"/restart/open/restart"+n_yyyy+n_mm+n_dd+"C"+numch+".bin") ## CaMa-Flood v4.1
...
def data_assim(yyyy,mm,dd): # new data assimilation function (2020/05/18)
    ...
    +" "+str(pm.conflag())+" "+(pm.calibrate())+" "+(pm.runname(pm.mode()))+" "+dd+" "+(pm.CaMa_out())+" "+(pm.CaMa_in())+" assim")  # online DA
    os.system(pm.DA_dir()+"/src/data_assim "+yyyy+mm+dd+" "\
    +str(pm.patch_size())+" "+str(pm.ens_mem(pm.mode()))+" "+nxtyyyymmdd+" "+pm.CaMa_dir()\
    +" "+str(pm.thersold())+" "+exp_dir+" "+pm.DA_dir()+" "+pm.patch_dir()+" "\
    +" "+pm.obs_dir()+" "+str(pm.rho())+" "+str(pm.sigma_b())\
    +" "+str(pm.conflag())+" "+(pm.calibrate())+" "+(pm.runname(pm.mode()))+" "+dd+" "+(pm.CaMa_out())+" "+(pm.CaMa_in())+" open")  # offline DA
    return 0
###########################
...
def make_initial_restart(): # updated the name
    ...
    for num in np.arange(1,pm.ens_mem()+1):
        ...
        infile_opn =exp_dir+pm.CaMa_in()+"/"+spinup_open+"/restart"+yyyy+mm+dd+".bin"
        infile_asm =exp_dir+pm.CaMa_in()+"/"+spinup_open+"/restart"+yyyy+mm+dd+".bin"
        ...
        os.system("cp ./"+pm.CaMa_out()+"/"+spinup_open+"/restart"+yyyy+mm_restart+dd+".bin  ./"+pm.CaMa_out()+"/"+spinup_open+"/restart"+yyyy+mm+dd+".bin")
        os.system("cp ./"+pm.CaMa_out()+"/"+spinup_open+"/restart"+yyyy+mm+dd+".bin ./"+pm.CaMa_in()+"/restart/open/restart"+yyyy+mm+dd+"C"+numch+".bin")
        os.system("cp ./"+pm.CaMa_out()+"/"+spinup_open+"/restart"+yyyy+mm+dd+".bin ./"+pm.CaMa_in()+"/restart/assim/restart"+yyyy+mm+dd+"A"+numch+".bin")
    return 0
###########################
    ...
    for looptype in listCA: # update on 2023/05/30
        ...
        for num in np.arange(1,pm.ens_mem()+1):
            ...
            for var in ["rivdph","sfcelv","outflw","storge"]: 
                shutil.copy("./"+pm.CaMa_out()+"/"+yyyy+mm+dd+CA+numch+"/"+var+yyyy+".bin","./assim_"+(pm.runname(pm.mode()))+"/"+var+"/"+looptype+"/"+var+yyyy+mm+dd+"_"+numch+".bin")
...
def make_restart(inputlist):
    ...
    +yyyy_n+mm_n+dd_n+" "+loop+" "+pm.CaMa_dir()+" "+pm.mapname()+" "+numch+" "+exp_dir+" "+pm.CaMa_out()+" "+pm.CaMa_in()+" "+(pm.runname(pm.mode())))
...
def read_sim_data(numch):
    ...
    mm="%02d"%(month)
    dd="%02d"%(date)
    file = "/work/a06/yingying/camada/HydroDA/src/"+pm.CaMa_out()+'/'+yyyy+mm+dd+"C"+numch+"/2019-sp1/o_rivdph2019.nc"
    print(file)
    return file
def calc_mean():
