...
def read_sim_data(ens):
    ...
    if 'round' in pm.__name__:
        file = pm.ILS_dir()+'/his'+yyyy+"/Qr00"+"_round"+str(pm.val_round(pm.val_mode()))+"_mean.nc"  # use the oneyear simulation to do anomaly
    else:
        file = pm.ILS_dir()+'/his'+yyyy+"/Qr00_mean.nc"
    return file
def calc_mean():
