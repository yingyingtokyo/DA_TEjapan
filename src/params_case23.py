...
# *************************************************************
def def_params():
    pm_line = 'import params_case23 as pm\n'
...
# 0. Obs preparation
def mode():
    return 8    # parameter to change assimilation mode
...
def flood_reg():
    ...
    return [38.73,40.51,139.666,140.9]
...
def starttime(DAhour=DA_leading()):
    if mode() == 8:
        return (2024,7,25,0) # start date: [year,month,date]
    else:
        return (2024,7,20,0) # start date: [year,month,date]

def endtime(DAhour=DA_leading()):
    #if DAhour == 'on':
        return (2024,7,26,0) # end date: [year,month,date]
    #else:
...
def outdir():
    if (val_mode()==0):
        return '/data50/yingying/HydroDA/out_all2/'
    else:
        return '/data50/yingying/HydroDA/out_case2_round'+ str(val_round(val_mode())) +'/'

def expout():
    if (val_mode()==0):
        return '../out_all2/'
    else:
        return '../out_case2_round'+ str(val_round(val_mode())) +'/'
