...
# *************************************************************
def def_params():
    pm_line = 'import params_real01 as pm\n'
...
def starttime(DAhour=DA_leading()):
    #if DAhour == 'on':
        return (2024,8,1,0) # start date: [year,month,date]
...
def endtime(DAhour=DA_leading()):
    #if DAhour == 'on':
        return (2019,8,6,0) # end date: [year,month,date]
...
def outdir():
    if (val_mode()==0):
        return '/data50/yingying/HydroDA/out_real/'
    else:
