...
OUTBASE="$EXP_DIR/$camaout_dir"
INBASE="../../$EXP_DIR/$camain_dir"
...
CROFPRE='Runoff_'
if [ $input = "tej" ];then
     CROFDIR="${INBASE}/inp/tej"    #   runoff directory
     CROFSUF="$ens_num.bin" 
elif [ $input = "ils" ];then
     CROFDIR="${INBASE}/inp/ils"    #   runoff directory
     CROFSUF="$ens_num.bin" 
elif [ $input = "msm" ];then
     CROFDIR="${INBASE}/inp/msm"    #   runoff directory
     CROFSUF="$ens_num.bin"
else
     CROFDIR="${INBASE}/inp/ils"    #   runoff directory
     CROFSUF="000.bin" 
...
EYEAR=`python ${DA_dir}/src/calc_end_hour.py $in_year $in_month $in_date $in_hour $dahour "year"`
EMON=`python ${DA_dir}/src/calc_end_hour.py $in_year $in_month $in_date $in_hour $dahour "month"`
EDAY=`python ${DA_dir}/src/calc_end_hour.py $in_year $in_month $in_date $in_hour $dahour "date"`
EHOUR=`python ${DA_dir}/src/calc_end_hour.py $in_year $in_month $in_date $in_hour $dahour "hour"`
...
# move restart file
RYR=$(printf "%04d" $((10#${EYEAR})))
RMO=$(printf "%02d" $((10#${EMON})))
RDA=$(printf "%02d" $((10#${EDAY})))
RHR=$(printf "%02d" $((10#${EHOUR})))
