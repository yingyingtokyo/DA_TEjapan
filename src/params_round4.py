...
# *************************************************************
def def_params():
    pm_line = 'import params_round4 as pm\n'
...
def starttime(DAhour=DA_leading()):
        ...
        # return (2024,1,20,0) # start date: [year,month,date]
    #else:
    #    return (2022,1,1)
...
def outdir():
    if (val_mode()==0):
        ...
        return '/data50/yingying/HydroDA/out_msm_round'+ str(val_round(val_mode())) +'/'

def expout():
    if (val_mode()==0):
        return '../out/'
    else:
        return '../out_msm_round'+ str(val_round(val_mode())) +'/'
